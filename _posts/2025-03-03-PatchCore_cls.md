...
추가된 17.7ms는 YOLOv8-cls가 카테고리를 분류하는 시간이다. 전체 추론 시간 534.2ms 중 약 3.3%에 해당하며, PatchCore의 FAISS KNN 검색(~500ms)에 비하면 무시할 수 있는 수준이다.
```
전체 파이프라인 시간 분해:
YOLOv8-cls 분류:  ~17ms  (3.3%)
Memory Bank 로드: ~10ms  (1.9%)
FAISS KNN 검색:  ~500ms (93.6%)
히트맵 생성:       ~7ms  (1.3%)
────────────────────────────
합계:             534ms
```

병목은 여전히 FAISS KNN 검색이다. 카테고리 자동 감지 추가로 인한 속도 오버헤드는 사실상 없다고 볼 수 있다.
...
반도체 웨이퍼, 특정 부품처럼 항상 동일한 제품만 검사하는 라인에서는 카테고리가 고정되어 있다. 이 경우 수동 선택 모드가 오히려 더 빠르고 안정적이다. YOLOv8-cls 추론 단계가 필요 없기 때문이다.

**▪ 다품종 검사 라인 (자동 감지 유리)**

여러 종류의 부품이 같은 컨베이어를 통과하는 다품종 소량 생산 라인에서 자동 감지가 빛난다. 작업자가 제품을 카메라 앞에 올려두기만 하면 시스템이 알아서 어떤 제품인지 판단하고 이상 탐지까지 수행한다.
```
실무 적용 시나리오:
[컨베이어 라인]
...
자동 감지의 신뢰도가 낮을 경우 수동 선택으로 fallback하는 로직을 추가하면 실무 안정성이 높아진다.
```python
