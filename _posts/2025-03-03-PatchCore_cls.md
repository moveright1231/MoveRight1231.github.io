...
동일한 이미지(bottle 불량 샘플)로 업데이트 전후를 비교했다.

![결과 비교](style/image/patchcore/c.png)
![결과 비교](style/image/patchcore/d.png)

| 항목 | 업데이트 전 (수동 선택) | 업데이트 후 (자동 감지) | 차이 |
|------|-------------------|-------------------|------|
| Anomaly Score | 0.9758 | 0.9758 | 동일 |
| 판정 결과 | 불량 (Anomaly) | 불량 (Anomaly) | 동일 |
| 추론 시간 | 516.5ms | 578.1ms | **+61.6ms** |
| 카테고리 선택 | 사용자 직접 선택 | 자동 감지 (YOLOv8-cls) | - |

**▪ Anomaly Score가 동일한 이유**

카테고리가 동일하게 bottle로 인식됐고, 같은 Memory Bank를 사용했기 때문에 PatchCore 추론 결과가 완전히 동일하다. 자동 감지 기능이 추론 품질에는 전혀 영향을 미치지 않는다.

**▪ 추론 시간 +61.6ms의 원인**

추가된 61.6ms는 YOLOv8-cls가 카테고리를 분류하는 시간이다. 전체 추론 시간 578.1ms 중 약 10.7%에 해당한다.
```
전체 파이프라인 시간 분해:
YOLOv8-cls 분류:  ~62ms  (10.7%)
Memory Bank 로드: ~10ms  ( 1.7%)
FAISS KNN 검색:  ~499ms  (86.3%)
히트맵 생성:       ~7ms  ( 1.2%)
────────────────────────────────
합계:             578ms
```

병목은 여전히 FAISS KNN 검색(86.3%)이다. YOLOv8-cls 추가로 인한 오버헤드(61.6ms)는 실무에서 허용 가능한 수준이며, FAISS GPU 전환 시 전체 속도를 대폭 개선할 수 있다.
...
반도체 웨이퍼, 특정 부품처럼 항상 동일한 제품만 검사하는 라인에서는 카테고리가 고정되어 있다. 이 경우 수동 선택 모드가 더 빠르고 안정적이다. YOLOv8-cls 추론 단계(62ms)가 불필요하기 때문이다.

**▪ 다품종 검사 라인 (자동 감지 유리)**

여러 종류의 부품이 같은 컨베이어를 통과하는 다품종 소량 생산 라인에서 자동 감지가 빛난다. 작업자가 제품을 카메라 앞에 올려두기만 하면 시스템이 어떤 제품인지 판단하고 이상 탐지까지 자동으로 수행한다.
```
다품종 라인 시나리오:
bottle → 자동 감지 → bottle Memory Bank → 이상 탐지
cable  → 자동 감지 → cable Memory Bank  → 이상 탐지
screw  → 자동 감지 → screw Memory Bank  → 이상 탐지
```

**▪ 신제품 추가 시 확장 방법**

새로운 제품 라인이 추가될 때 기존 시스템 전체를 재학습할 필요가 없다.
```bash
# 1. 새 제품 정상 이미지 200장 수집
# 2. PatchCore Memory Bank 학습 (약 1분)
python train.py --category new_product --exp exp1

# 3. YOLOv8-cls 재학습 (기존 15개 + 신제품 포함)
python prepare_cls_data.py
python train_cls.py
...
기존 15개 카테고리의 Memory Bank는 그대로 유지되고 새 카테고리만 추가되므로, 기존 검사 성능에 영향을 주지 않는다.

**▪ 분류 신뢰도 기반 안전장치**

자동 감지 신뢰도가 낮을 경우 수동 선택으로 fallback하는 로직을 추가하면 실무 안정성이 높아진다.
```python
if category_confidence < 0.80:
    # 신뢰도 낮음 → 수동 선택 요청
    return {"status": "uncertain", "top3": top3_categories}
else:
    # 신뢰도 충분 → 자동 추론 진행
    return run_patchcore(detected_category)
```
